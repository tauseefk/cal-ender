...
pub fn get_subtree_depth_transforms(
    ...
    // The cluster height is the shared column grid, so siblings on different-depth
    ...
    match stack_position < 1.0 {
        ...
        false => {
            let width = match subtree_depth > 0.0 {
                // non-leaf: one column plus a half-column overlap into its child, so
                // nesting stays visible but the block never reaches the right edge.
                true => 1.5 / height,
